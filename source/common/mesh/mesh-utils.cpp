...
our::Mesh *our::mesh_utils::loadOBJ(const std::string &filename) {
    ...
    if (!tinyobj::LoadObj(&attrib, &shapes, &materials, &warn, &err, filename.c_str(), "assets/models")) {
...
our::Mesh *our::mesh_utils::sphere(const glm::ivec2 &segments) {
    ...
    ///  creates triangles by calculating the correct vertex indices for each pair of adjacent vertices
    ///  (forming a quad of sorts).
    /// These are stored in the elements array following the order of CCW winding from the outside.
