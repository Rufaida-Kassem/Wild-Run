...
namespace our {
    ...
    glm::mat4 CameraComponent::getProjectionMatrix(glm::ivec2 viewportSize) const {
        ...
        // It takes fovY, aspect, near and far as parameters
        // NOTE: The aspect ratio is the width divided by the height
        float aspect = viewportSize.x / (float)viewportSize.y;
        if(cameraType == CameraType::ORTHOGRAPHIC){
            return glm::ortho(-orthoHeight * aspect / 2.0f, orthoHeight * aspect / 2.0f, -orthoHeight / 2.0f, orthoHeight / 2.0f, near, far);
        } else {
